...

{
   ...
   if ( ! pio_bmide_base_addr )
      return 0;
   x = in8(pio_bmide_base_addr + BM_COMMAND_REG);
   return x;
}
...

{
   ...
   if ( ! pio_bmide_base_addr )
      return 0;
   x = in8( pio_bmide_base_addr + BM_STATUS_REG );
   return x;
}
...

{
   ...
   if ( ! pio_bmide_base_addr )
      return;
   out8( pio_bmide_base_addr + BM_COMMAND_REG, x);
}
...

{
   ...
   if ( ! pio_bmide_base_addr )
      return;
   out8( pio_bmide_base_addr + BM_STATUS_REG, x);
}
...
static unsigned int pio_inword( unsigned char addr )
{
   return in16(pio_reg_addrs[addr]);
}
...
static unsigned long pio_indword( unsigned char addr )
{
   return in32(pio_reg_addrs[addr]);
}
...

{
   ...
   while ( wordCnt > 0 )
   {
      * (u16 *) bufAddr = pio_inword( addrDataReg );
      bufAddr += 2;
      wordCnt -- ;
   }
}
...

{
   ...
   while ( wordCnt > 0 )
   {
      pio_outword( addrDataReg, * (u16 *) bufAddr );
      bufAddr += 2;
      wordCnt -- ;
   }
}
